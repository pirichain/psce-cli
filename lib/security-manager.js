...
class PSCESecurityManager {
  ...
  async storeNetwork(name, networkData) {
    ...
    // For now, store without encryption to avoid session issues
    await keytar.setPassword(
      this.SERVICE_NAME,
      key,
  ...
  async getNetwork(name) {
    ...
    try {
      const data = await keytar.getPassword(this.SERVICE_NAME, key);
      if (!data) {
        return null;
      }
  ...
  async setActiveNetwork(name) {
    ...
    try {
      await keytar.setPassword(this.SERVICE_NAME, key, name);
  ...
  async getActiveNetwork() {
    ...
    try {
      const activeNetwork = await keytar.getPassword(this.SERVICE_NAME, key);
  ...
  async setActiveAddress(address) {
    ...
    try {
      await keytar.setPassword(this.SERVICE_NAME, key, address);
  ...
  async getActiveAddress() {
    ...
    try {
      const activeAddress = await keytar.getPassword(this.SERVICE_NAME, key);
