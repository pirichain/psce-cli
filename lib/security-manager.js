...
class PSCESecurityManager {
  ...
  // Safe keytar wrapper with findCredentials
  async safeKeytarFindCredentials(service) {
    try {
      if (this.isLinuxWithoutDisplay) {
        return await this.fileBasedFindCredentials();
      }
  ...
  async fileBasedFindCredentials() {
    ...
    for (const [account, password] of Object.entries(data)) {
      credentials.push({
        account: account,
        password: password,
      });
  ...
  async decryptWallet(address, masterPassword) {
    const walletKey = `wallet:${address}`;
    const storedData = await this.safeKeytarGet(this.SERVICE_NAME, walletKey);
  ...
  async getAllNetworks() {
    ...
    try {
      ...
      const credentials = await this.safeKeytarFindCredentials(
        this.SERVICE_NAME
      );
      const networks = {};
  ...
  async detectKeytarFailure() {
    try {
      await this.safeKeytarFindCredentials(this.SERVICE_NAME);
      return false;
    } catch (error) {
      return (
